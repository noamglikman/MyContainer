...
public:
    ...
    /**
     * @brief Returns an iterator to the first element in side-cross order.
     *
     * @param container The container to iterate over.
     * @return SideCrossOrder iterator pointing to the start of iteration.
     */
    static SideCrossOrder begin(const MyContainer<T>& container) {
        return SideCrossOrder(container);
    }

    /**
     * @brief Returns an iterator representing the end of the iteration.
     *
     * @param container The container to iterate over.
     * @return SideCrossOrder iterator representing the past-the-end position.
     */
    static SideCrossOrder end(const MyContainer<T>& container) {
        SideCrossOrder it(container);
        it.index = it.sortedData.size();
        return it;
    }
};

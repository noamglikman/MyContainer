...
public:
    ...
    return index == other.index;
        ...
        /**
     * @brief Returns an iterator pointing to the first element (the middle) in middle-out order.
     *
     * @param c The container to iterate over.
     * @return MiddleOutOrder iterator pointing to the middle element.
     */
    static MiddleOutOrder begin(const MyContainer<T>& c) {
        return MiddleOutOrder(c);
    }

    /**
     * @brief Returns an iterator representing the end (past-the-last element) in middle-out order.
     *
     * @param c The container to iterate over.
     * @return MiddleOutOrder iterator pointing past the last element.
     */
    static MiddleOutOrder end(const MyContainer<T>& c) {
        MiddleOutOrder it(c);
        it.index = it.data.size();  // past-the-last position
        return it;
    }
